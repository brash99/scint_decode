...
Int_t pedestal_ends[NUMADCSLOTS][NUMCHANA];
...
void clear() {
  ...
  //MAPPING
  memset(adcmapped, 0, MAXPMTIDS*NPIXELS*sizeof(Int_t));
  memset(adcrawmapped, 0, MAXPMTIDS*NPIXELS*sizeof(Int_t));
  memset(tdctmapped, 0, MAXPMTIDS*NPIXELS*sizeof(Int_t));
  memset(tdclmapped, 0, MAXPMTIDS*NPIXELS*sizeof(Int_t));
}
...
void decode (Int_t* data, TTree *tree) {
  ...
  for (Int_t j=0;j<NUMADCSLOTS;j++){
    for (Int_t ii=0;ii<NUMCHANA;ii++){
      tree_adc_hits[j][ii] = -100;
    }
  }
  for (Int_t j=0;j<NUMTDCSLOTS;j++){
    for (Int_t ii=0;ii<NUMCHANT;ii++){
  ...
  for (Int_t j = index+1; j < indexlast; j++) {
	...
	Int_t ped = pedestal_threshold[tree_adc_slot][tree_adc_chan];

        tree_adc_chan = ichan;
        tree_adc_hits[tree_adc_slot][tree_adc_chan] = rdata;
	tree_adc_pedestal_chan[tree_adc_slot][tree_adc_chan] = ped;

	tree_adc_hits_minus_pedestal[tree_adc_slot][tree_adc_chan] = rdata - ped;
  ...
  // is_good = overall flag for the event
  // is_good = false; // this line is the correct thing to do, but...  
  is_good = true; // ...let's just pretend that it is good anyway, for debug
  if (debug && is_good) cout << "WARNING: is_good forced to true. Just so you know..."<<endl;
  ...
  for (Int_t j = indexlast+1; j < indexend; j++) {
	...
	if (debug==2) printf("data[%d] = 0x%x = (dec) %d %d  \n",j,data[j],(data[j]&0xfe0000)>>17,data[j]&0xffff);
      ...
      // if pedestal event, do NOT process
      // if (adcdat[slotindex][ichan] <= pedestal_ends[slotindex][ichan])rdata = -100;
  ...
  // mapping
  // for (int pmt = 0; pmt < map_len; pmt++){
  //   for (int pixel = 0; pixel < 16; pixel++){
  //     int adc_slot = map_adc_slot[pmt][pixel];
  //     int adc_chan = map_adc_chan[pmt][pixel];
  //     int tdc_slot = map_tdc_slot[pmt][pixel];
  //     int tdc_chan = map_tdc_chan[pmt][pixel];
  //     int pmtID = map_pmt_id[pmt];
      
  //     adcmapped   [pmtID][pixel+first_pixel]=tree_adc_hits_minus_pedestal[adc_slot][adc_chan];
  //     adcrawmapped[pmtID][pixel+first_pixel]=tree_adc_hits[adc_slot][adc_chan];
  //     tdctmapped  [pmtID][pixel+first_pixel]=tree_tdc_hits[tdc_slot][tdc_chan];
  //     tdclmapped  [pmtID][pixel+first_pixel]=tree_tdc_le_hits[tdc_slot][tdc_chan];
  //   }
  // }
...
void readpedsup(){
  ...
  for (Int_t i = 0; i < NUMADCSLOTS;i++) {
    ...
    for (Int_t j = 0; j < NUMCHANA;j++) {
      ...
      pedestal_ends[i][j] = atoi(pedend);
      if (debug) cout << "readpedsup(): chan = "<< j<<" thres = '" << thres << "' value = " << pedestal_threshold[i][j] <<" pedend = " << pedestal_ends[i][j] <<endl;
    }
  }
